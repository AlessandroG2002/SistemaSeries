...
@Named
@ViewScoped
public class AvaliacaoFilmeController extends Controller<Avaliacao> implements Serializable {
...
//		try {
//			setEntity(em.find(Avaliacao.class, repo.findByUsuarioAndMidia((Usuario) Session.getInstance().get("usuarioLogado"), (Midia) Session.getInstance().get("filmeSelecionado"))));
...
//		}
		...
		entity.setMidia((Midia) Session.getInstance().get("filmeSelecionado"));
		return entity;
	}
	...
	public void abrirAvaliacao() {
		...
		PrimeFaces.current().dialog().openDynamic("avaliacaofilme", options, null);
	}
